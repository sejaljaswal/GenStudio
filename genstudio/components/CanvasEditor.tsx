 ...
 * Works in all browsers — no need for canvas.toBlob().
 */
...
export function CanvasEditor({ imageUrl, generationId, onClose }: CanvasEditorProps) {
  ...
  const saveEditedImage = async () => {
    ...
    setIsSaving(true);
    try {
      // Use toDataURL (always available on Fabric canvas) then convert to Blob
      ...
      // Update SWR cache so gallery reflects the new image immediately
      await mutate(
        "/api/generations",
        (gens: any) => {
  ...
  useEffect(() => {
    ...
    const initFabric = async () => {
      ...
      try {
        ...
        fCanvas.setDimensions({ width: scaledWidth, height: scaledHeight });
  ...
  const applyCrop = async () => {
    ...
    // Export only the cropped region using toDataURL (always available)
    ...
    // Load cropped image back onto a fresh canvas
    const fabric = await import("fabric");
    const croppedImg = await fabric.FabricImage.fromURL(croppedDataURL, {
      crossOrigin: "anonymous",
    });

    // Clear canvas and resize to crop dimensions
    canvas.clear();
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-black/90 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-background/95 backdrop-blur border border-border/40 p-2 rounded-xl flex items-center gap-1.5 shadow-xl overflow-x-auto max-w-[95vw]">
        {/* Save Edit button (upload to server) — hidden while cropping */}
        {!isCropping && (
          <Button
            variant="default"
            size="sm"
